...
def build_model(X, y):
    ...
    conv2_1 = Conv1D(filters = 20, kernel_size = 3, activation = 'relu')(inputs2)
    conv2_2 = Conv1D(filters = 20, kernel_size = 3, activation = 'relu')(conv2_1)
    flat2 = Flatten()(conv2_2)
    # channel 3
    inputs3 = Input(shape=(X.shape[1], 1))
    conv3_1 = Conv1D(filters = 20, kernel_size = 5, activation = 'relu')(inputs3)
    conv3_2 = Conv1D(filters = 20, kernel_size = 5, activation = 'relu')(conv3_1)
...
start = timer()
pred = model.predict([testX[:,:,:1],testX[:,:,1:2],testX[:,:,2:]]) # get the probabilities of each class
end = timer()
...
cm_analysis(testy, pred_y, filename='DP13_pics/DP13_multi_CNN_CM.png',
