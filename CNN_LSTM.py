...
def build_model(X, y):
    ...
    # define model
    model = Sequential()
    model.add(TimeDistributed(Conv1D(filters=35, kernel_size=3, activation='relu', input_shape=(X.shape[1], X.shape[2]))))
    ...
    #plot the model
    plot_model(model, show_shapes = True, to_file='DP13_pics/DP13_CNN_LSTM.png')
    return model
...
cm_analysis(testy, pred_y, filename='DP13_pics/DP13_CNN_LSTM_CM.png',
